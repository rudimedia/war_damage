...
def list_label_dates(city, geometry=None):
    ...
    pattern = os.path.join(
        LABELED_FOOTPRINT_DIR, f"{city}_*_1_{geometry}.csv")
...
# --------------------------------------------------------------------------
# Label files
# Notebook 0 writes the project target and audit columns. Loading preserves any
...
def label_path(city, date, geometry=None):
    geometry = geometry or PREP["label_geometry"]
    return os.path.join(
        LABELED_FOOTPRINT_DIR, f"{city}_{date}_1_{geometry}.csv")
...
def _post_tag(window_days=None):
    ...
    default post-composite variant.
    """
...
def post_arrays(d, date, window_days="auto"):
    ...
    if window_days == "auto":
        window_days = None if date in d["dates"] else temporal_scene_count()
...
def temporal_features(scores_by_offset):
    ...
    and post-event persistence without adding redundant aggregate features.
    """
