"""
Sentinel-1 SAR preprocessing, training and experiment utilities.

The project notebooks import this file so the city registry, file paths, data
loading, models and augmentation helpers have one shared implementation.

Expected folder layout on Google Drive::

    War-Damage-Detection/
        shared/data/labeled_footprints/  output from the shared notebook 0
        sentinel_sar/
            pipeline.py                  this file
            data/rasters/                Sentinel-1 exports from notebook 1
            data/processed/              row-aligned patches from notebook 1
            experiments/                 one folder per notebook 2 run
...
The division of labour between the notebooks:
    ...
    2_training_evaluation  trains models on the saved patches, selects them on
                           validation data and evaluates the frozen pipeline
    shared/3_interactive_map applies a trained Sentinel or Planet experiment
                             and shows its predictions
...
# --------------------------------------------------------------------------
# Paths. Environment variables are optional overrides for non-Colab runs.
# In the standard Drive layout these defaults resolve relative to this file.
# --------------------------------------------------------------------------

SENSOR_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.abspath(os.environ.get(
    "WAR_DAMAGE_ROOT", os.path.dirname(SENSOR_DIR)))
SHARED_DATA_DIR = os.path.abspath(os.environ.get(
    "WAR_DAMAGE_SHARED_DATA", os.path.join(PROJECT_ROOT, "shared", "data")))
LABELED_FOOTPRINT_DIR = os.path.abspath(os.environ.get(
    "WAR_DAMAGE_LABELED_FOOTPRINT_DIR",
    os.path.join(SHARED_DATA_DIR, "labeled_footprints")))
# DATA_DIR remains an alias because older helper names use it internally.
DATA_DIR = LABELED_FOOTPRINT_DIR
RASTER_DIR = os.path.abspath(os.environ.get(
    "S1_RASTER_DIR", os.path.join(SENSOR_DIR, "data", "rasters")))
...
PROCESSED_DIR = os.path.abspath(os.environ.get(
    "PATCH_DIR", os.path.join(SENSOR_DIR, "data", "processed")))
EXPERIMENTS_DIR = os.path.abspath(os.environ.get(
    "S1_EXPERIMENTS_DIR", os.path.join(SENSOR_DIR, "experiments")))
...
PREP = {
    # Sensor provenance is retained in processed filenames. This pipeline is S1-only.
    "sensors": ["s1"],
...
#     both true and not in conflict. Notebook 2 validates its hand-written
#     split with check_split_holdout(); held_out_cities() supplies the source
#     of truth for that guard.
#
# The pre-event window and every post-event search must be usable:
#
#     pre  = the configured number of months ENDING at war_start
#     post = the closest Sentinel-1 acquisition day(s) within
#            +/- post_search_days of each target date
#
# A search that reaches before war_start could select pre-war imagery for a
# post-event observation. Overlapping searches for consecutive assessments can
# also reuse an acquisition. Sentinel-1 IW barely exists before 2014-10-01.
# check_city_windows() reports all three conditions before notebook 1 exports.
# --------------------------------------------------------------------------
...
def add_city(name, label_dates, war_start, role="develop"):
    """Register another city for this session, then rerun notebook 1."""
...
# --------------------------------------------------------------------------
# Channel names. Selection happens by name rather than position.
#
...
def _s1_sensors(sensors=None):
    """Return the configured sensor list and enforce the S1-only pipeline."""
    selected = list(PREP["sensors"] if sensors is None else sensors)
    if selected != ["s1"]:
        raise ValueError(
            f"The preprocessing pipeline supports only ['s1']; got {selected}.")
    return selected
...
def stored_channels(phase, sensors=None):
    ...
    _s1_sensors(sensors)
    return [name for name in S1_CHANNELS if f"_{phase}_" in name]


def wanted_channels(features=None, sensors=None):
    """Selected pre-event and post-event S1 channels in canonical order."""
    _s1_sensors(sensors)
    ...
    for phase in ["pre", "post"]:
        if not features.get(f"{phase}_event", True):
            continue
        names.extend(stored_channels(phase, sensors))
    return names


# --------------------------------------------------------------------------
# Label files
# Notebook 0 writes the project target and audit columns only. Optional columns
# present in older CSVs remain readable because loading preserves unknown fields.
# --------------------------------------------------------------------------
...
def load_labels(city, date, geometry=None):
    """Read one notebook-0 label CSV into a GeoDataFrame with centroids."""
    path = label_path(city, date, geometry)
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"{path} not found. Move the existing Data/own_footprints folder "
            "to shared/data/labeled_footprints in Drive, or run "
            "shared/0_build_labeled_footprints.ipynb for this city/date.")
...
def prep_tag():
    ...
    return (f"p{PREP['patch_size']}_{PREP['label_geometry']}"
            f"_{label_tag}_n{PREP['n_sample']}_{''.join(sensors)}")
...
def post_primary_raster_path(city, date, sensor, window_days=None):
    ...
    from the configured fallback dates. When fallbacks are enabled, this
    companion raster contains only the closest acquisition(s), allowing
    notebook 1 to measure how many pixels and patches the fallback resolved.
    """
...
def cut_patches(city, date, lonlat, patch=None, window_days=None):
    ...
    sensors = _s1_sensors()
    sources = ([pre_raster_path(city, s) for s in sensors] +
               [post_raster_path(city, date, s, window_days) for s in sensors])
    for path in sources:
...
def neighbour_features(xy, scores, ks=(8, 32)):
    """Compact spatial features derived from the assessment-date CNN score.
    ...
    n = len(scores)
    feats = {"score_t0": np.asarray(scores, np.float32)}
    if n < 2:
        for k in ks:
    ...
    _, nn = tree.query(xy, k=max_k)         # column 0 is the point itself

    for k in ks:
        k_eff = min(k, max_k - 1)
        nb = scores[nn[:, 1:k_eff + 1]]
        feats[f"k{k}_mean"] = nb.mean(axis=1)
        feats[f"k{k}_std"] = nb.std(axis=1)
...
def temporal_features(scores_by_offset):
    """Return the six temporal predictors in the compact XGBoost schema.

    Offset 0 is the labelled date. Every array must be the same length and
    aligned to the same buildings. The four nonzero observations are supplied
    independently. Two explicit change features summarize the pre-assessment rise
    and post-event persistence without adding the larger redundant aggregate
    set used by earlier experiments.
    """
    required = (-24, -12, 0, 12, 24)
    offsets = tuple(sorted(int(offset) for offset in scores_by_offset))
    if offsets != required:
        raise ValueError(
            "The compact temporal feature schema requires offsets "
            f"{required}; got {offsets}.")

    scores = {
        offset: np.asarray(scores_by_offset[offset], np.float32)
        for offset in required
    }
    lengths = {len(values) for values in scores.values()}
    if len(lengths) != 1:
        raise ValueError("All temporal score arrays must have the same length")

    def row_mean(left, right):
        pair = np.column_stack([left, right])
        valid = np.isfinite(pair)
        count = valid.sum(axis=1)
        total = np.where(valid, pair, 0.0).sum(axis=1)
        return np.divide(
            total, count, out=np.full(len(pair), np.nan, np.float32),
            where=count > 0)

    pre_mean = row_mean(scores[-24], scores[-12])
    post_mean = row_mean(scores[12], scores[24])
    feats = {
        "score_t-24": scores[-24],
        "score_t-12": scores[-12],
        "score_t+12": scores[12],
        "score_t+24": scores[24],
        "pre_change": scores[0] - pre_mean,
        "post_persistence": post_mean - scores[0],
    }
...
class SiameseCNN(nn.Module):
    ...
    def __init__(self, n_channels, width=8, depth=2, dropout=0.4,
...
LEGACY_RASTER_TAG = "pre12m_backward1m"


def legacy_raster_path(city, date, sensor):
    return os.path.join(
        RASTER_DIR, f"{city.lower()}_{date}_{sensor}_{LEGACY_RASTER_TAG}.tif")
